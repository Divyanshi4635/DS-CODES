...
template <class T>
lowertriangle<T>::lowertriangle(int N)
{
	if(N<1)
...
template <class T>
T lowertriangle<T>::get(int i,int j,bool flag)const
{
	if(i<0||j<0||i>n||j>n)
	...
	{
		if(i>=j)
			cout<<arr[n*(n+1)/2 - (n-j+1)*(n-j+2)/2 + i-j];
		else
			cout<<"0";
	}
}
...
template <class T>
void lowertriangle<T>::set(int i,int j, T& val, bool flag)
{
	if(i<0||j<0||i>n||j>n)
		cout<<"Array Index out of bounds";
